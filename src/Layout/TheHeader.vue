<template>
  <div class="layout-topbar">
    <div class="topbar-left">
      <a class="menu-button" @click="menuButtonTapped">
        <i :class="'pi pi-chevron-' + menuIconArrow"></i>
      </a>
      <span class="topbar-separator"></span>

      <div class="layout-breadcrumb viewname" style="text-transform: uppercase">
        <span v-if="$route.meta.label">{{ $route.meta.label }}</span>
      </div>
    </div>
    ...
    <div class="topbar-right">
      <ul class="topbar-menu">
        <li class="search-item">
          <a tabindex="0" @click="searchButtonTapped">
            <i class="pi pi-search"></i>
          </a>
        </li>

        <li
          :class="{ 'active-menuitem fadeInDown': topBarMenuIsActive }"
          class="profile-item"
        >
          <a href="#">
            <span class="profile-name">The Movies DB</span>
          </a>
        </li>
...
export default  defineComponent({
  ...
  methods: {
    menuButtonTapped(){
      ...
      this.menuIconArrow = this.menuIconArrow === 'left' ? 'right' : 'left'
    },
    searchButtonTapped(){
      this.$router.push({
        name:'Search'
